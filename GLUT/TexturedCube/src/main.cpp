...
static char WINDOW_TITLE[] = "TexturedCube";
static char VERTEX_SHADER_FILE_PATH[] = "C:/workspace/learning/OpenGL/TexturedCube/shaders/vertex_shader.glsl";
static char FRAGMENT_SHADER_FILE_PATH[] = "C:/workspace/learning/OpenGL/TexturedCube/shaders/fragment_shader.glsl";
...
GLuint TEXTURE_COORDINATES_VBO_ID;	// This will identify our vertex buffer object for texture coordinates
...
int main(int argc, char** argv)
{
	...
	camera = new Camera();
    camera->setCentre(glm::vec3(0.0f, 0.0f, 4.0f));
	...
	glGenBuffers(1, &TEXTURE_COORDINATES_VBO_ID);
	glBindBuffer(GL_ARRAY_BUFFER, TEXTURE_COORDINATES_VBO_ID);
	glBufferData(GL_ARRAY_BUFFER,	// Type of buffer
				cubeObject->getModel()->getVertexCount() * sizeof(glm::vec2), // size of buffer
				cubeObject->getModel()->getVertexTextureCoordinatesArray(), // pointer to contents of buffer
				GL_STATIC_DRAW); // nature of data in buffer
	...
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA,
					cubeObject->getModel()->getTextureImage()->getWidth(),
					cubeObject->getModel()->getTextureImage()->getHeight(),
					0, GL_RGBA, GL_UNSIGNED_BYTE,
					cubeObject->getModel()->getTextureImage()->getBits());
	...
	glDeleteBuffers(1, &TEXTURE_COORDINATES_VBO_ID);
